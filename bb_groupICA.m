...
keep_var = [0.6 0.65 0.7 0.75 0.8 0.85 0.9 0.95];
...
for i = 1:length(nComp)
    ...
    PCA_matrix = bsxfun(@plus, PCA_matrix,mean(hybrid_pca,2)); % plug the mean back
    ...
    [iq,ica_weights,w,ica_timeCourses] = icassoShow(sR,'L',nComp(i));    
    %close all   
...
save('groupICA_results_ssr.mat','groupICA_results')
